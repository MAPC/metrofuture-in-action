...
set :application,   "projects.metrofuture.org"
set :scm,           :git 
set :repository,    "git@github.com:MAPC/metrofuture-in-action.git" 
...
# Use our ruby-2.0.0@metrofuture-in-action gemset 

default_environment['PATH']         = "/usr/local/rvm/gems/ruby-2.0.0-p247@metrofuture-in-action/bin:/usr/local/rvm/gems/ruby-2.0.0-p247@global/bin:/usr/local/rvm/rubies/ruby-2.0.0-p247/bin:/usr/local/rvm/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/usr/games:$PATH"
default_environment['GEM_HOME']     = "/usr/local/rvm/gems/ruby-2.0.0-p247@metrofuture-in-action"
default_environment['GEM_PATH']     = "/usr/local/rvm/gems/ruby-2.0.0-p247@metrofuture-in-action:/usr/local/rvm/gems/ruby-2.0.0-p247@global"
...
namespace :deploy do
  ...
  task :restart, except: { no_release: true } do
    begin
      run "kill -s USR2 `cat /tmp/unicorn.metrofuture-in-action.pid`"
    rescue
  ...
  task :stop, except: { no_release: true } do
    run "kill -s QUIT `cat /tmp/unicorn.metrofuture-in-action.pid`"
  end
